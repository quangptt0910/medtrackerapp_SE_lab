...
class MedicationModelTests(TestCase):
    ...
    def test_expected_doses(self):
        med = Medication.objects.create(name="Enzym", dosage_mg=100, prescribed_per_day=3)
        days = 5
    ...
    def test_adherence_rate_period_some_dose_taken(self):
        ...
        DoseLog.objects.create(medication=self.med, taken_at=datetime(2025,10,1,0,0))
        DoseLog.objects.create(medication=self.med, taken_at=datetime(2025,10,1,20,10))
        DoseLog.objects.create(medication=self.med, taken_at=datetime(2025,10,2,10,10))
        DoseLog.objects.create(medication=self.med, taken_at=datetime(2025,10,2,20,10))
        DoseLog.objects.create(medication=self.med, taken_at=datetime(2025,10,3,23,59))
...
class DoseLogModelTests(TestCase):
    ...
    def test_doselog_str_taken(self):
        ...
        log = DoseLog.objects.create(medication=self.med, taken_at=dose_time, was_taken=True)
        self.assertIn("Ibuprofen", str(log))
        self.assertIn("Taken", str(log))

    def test_doselog_str_missed(self):
        dose_time = timezone.make_aware(datetime(2025, 10, 1, 10, 30))
        log = DoseLog.objects.create(medication=self.med, taken_at=dose_time, was_taken=False)
        self.assertIn("Ibuprofen", str(log))
    ...
    def test_doselog_cascade_delete(self):
        ...
        DoseLog.objects.create(medication=self.med, taken_at=dose_time - timedelta(hours=1))
